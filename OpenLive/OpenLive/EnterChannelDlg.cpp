...
BEGIN_MESSAGE_MAP(CEnterChannelDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON_JOINSUB, &CEnterChannelDlg::OnBnClickedBtnjoinSubChannel)
...
void CEnterChannelDlg::InitCtrls()
{
	...
	m_btnJoin.MoveWindow(ClientRect.Width() / 2 - 180, 310, 170, 36, TRUE);
	m_btnJoinSub.MoveWindow(ClientRect.Width() / 2 + 10, 310, 170, 36, TRUE);
	...
	m_btnJoinSub.SetBackColor(RGB(0x00, 0xA0, 0xE9), RGB(0x05, 0x78, 0xAA), RGB(0x05, 0x78, 0xAA), RGB(0xE6, 0xE6, 0xE6));
	m_btnJoinSub.SetFont(&m_ftBtn);
	m_btnJoinSub.SetTextColor(RGB(0xFF, 0xFF, 0xFF), RGB(0xFF, 0xFF, 0xFF), RGB(0xFF, 0xFF, 0xFF), RGB(0xCC, 0xCC, 0xCC));
	m_btnJoinSub.SetWindowText(LANG_STR("IDS_CHN_BTJOINSUB"));
...
void CEnterChannelDlg::OnBnClickedBtnjoinSubChannel()
{
	// TODO:  在此添加控件通知处理程序代码
	...
	GetParent()->SendMessage(WM_JOINSUBCHANNEL, 0, 0);
}
